...
We will be using .Rmd for our sessions with NHANES data.
...
NHANES has collected accelerometer data during the below years: 1.
2003-2004 (Hip-worn ActiGraph) 2. 2005-2006 (Hip-worn ActiGraph) *3.
2011-2012 (Wrist-worn ActiGraph)* *4. 2013-2014 (Wrist-worn ActiGraph)*
...
Now we can just work with NHANES_data going forward.
...
compliance, - Device failure or sensor error, - Premature battery
depletion. - etc.

Therefore before we explore the data or run any epi analysis, we will
first clean our dataset using the following checks:
...
**Exercise 1: If you usually do any additional cleaning in your group,
please feel free to discuss it with the person next to you.**\
An interesting review at the bottom (Pulsford, 2023) discusses different
cleaning approaches.
...
demographics. We've used a few ways of doing this, but feel free to make
your own based on how you would normally do this.
...
ggplot(NHANES_data) +
  geom_histogram(aes(x=bmi), binwidth = 2.5) + theme_bw()

ggplot(NHANES_data) +
  geom_histogram(aes(x=age), binwidth = 5, fill = "grey80", colour = "black") + theme_bw()
...
NHANES_data %>%
  ...
  ggplot() +
  geom_bar(aes(x=bmi_cat)) + theme_bw()

# As we plan on doing some epi analysis, we should also look at the number of participants in the sample who have died according to the mortality data
sum(NHANES_data$death_f == TRUE, na.rm = TRUE) # 479 deaths is what we got
```
...
```{r Inspect cadence data}
NHANES_cad <- NHANES_data %>%
  select(CadencePeak1Adjusted.steps.min.,
  ...
  ggplot(NHANES_cad, aes(x=value)) + 
    ...
    theme_bw() + labs(x = "Cadence (steps/min)", y = "Frequency")
rm(NHANES_cad)
```

The above should hopefully make a bit more sense later on if you are
less familiar with them.

### Exercise 2: Self-exploration of the data
...
Some good starting variables to explore: - WearTime.days. -
ENMOAdjusted.mg. - StepsDayAvgAdjusted - StepsDayMedAdjusted
...
1.  Anything interesting you found in the data e.g. how active they are,
    differences between weekend and weekday steps.
2.  Any potential issues you encountered with the current dataset that
    may need considering in tomorrows epidemiology analysis.
